...
{
	if (argument_count > 1) var _npc_id = argument[1]; else _npc_id = -1
	switch(argument[0])
	{
		case 0: break;
		case 1: new_textbox("response 1",1,_npc_id,1,["2:go 2","3:go 3"]); break;
		case 2: new_textbox("response 2",1,_npc_id,2); break;
		case 3: new_textbox("response 3"); break;
		case 4: new_textbox("response 4"); break;
		case 5: 
		{
			new_textbox("Thank you so much! I'll be awaiting your return eagerly!",1,_npc_id)
			global.quest_status[? "the_hat_quest"] = 1
		}break;
		case 6: new_textbox(":(",1,_npc_id) break;
