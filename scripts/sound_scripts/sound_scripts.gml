...
function play_sound(_sound, _randomize_pitch, _randomness)
{
	...
	if (sound_settings && !audio_is_playing(sound_settings))
	{
		...
		{
			if (_randomize_pitch) sound_settings.pitch = random_range(1 - _randomness, 1 + _randomness)
		}
...
/// @arg _randomize_pitch Adds randomness to pitch 
/// @arg _randomness Range of pitch randomness
function play_sound_struct(_sound, _randomize_pitch, _randomness)
{
	if (!audio_is_playing(_sound.sound))
	{
		if (argument_count < 1) // randomize pitch
		{
			if (_randomize_pitch) _sound.pitch = random_range(1 - _randomness, 1 + _randomness)
		}
